...
class CompressorChain:
    ...
    def train(self, alphabet_size, iter_train, iter_val):
        for c in self.compressors:
            ...
            iter_train = self._lazy_iterate(c, iter_train, True)
            iter_val = self._lazy_iterate(c, iter_val, False)
    ...
    def _lazy_iterate(self, compressor, old_iter, shuffle):
        ...
        def new_iter():
            ...
            if storage.tell() != 0:
                ...
                if shuffle:
                    ...
                    self._shuffle_file_lines(storage)
    ...
    def _shuffle_file_lines(self, file):  # (approximately) shuffle the lines of a (large) file
        buffer_sz = 1024 * 8  # number of lines to read per chunk
        end = False
        while not end:
            buffer = []
            pos = file.tell()
            ...
            file.seek(pos)
            file.write("\n".join(buffer))  # should be identical size in binary
